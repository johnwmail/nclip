...
test_small_content_full_render() {
    ...
    local paste_url
    paste_url=$(curl -sS -X POST -H "Content-Type: text/plain" --data-binary "$test_content" "$NCLIP_URL/")
...
test_large_content_preview() {
    ...
    local paste_url
    paste_url=$(curl -sS -X POST -H "Content-Type: text/plain" --data-binary "@$temp_file" "$NCLIP_URL/")
